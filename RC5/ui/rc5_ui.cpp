...
//
//  ВАЖНО: этот файл НЕ знает о классе RC5 напрямую. Вся работа с шифром
//  идёт через Rc5Module — обёртку, которая грузит rc5.dll/rc5.so в рантайме
//  и вызывает функции через указатели (см. rc5_module_api.h, rc5_capi.h).
// =============================================================================
...
void runRC5() {
    ...
    while (running) {
        ...
        // Создаём объект RC5 через библиотеку (handle — непрозрачный указатель)
        ...
        // Освобождаем объект RC5 после использования
        mod.destroy(handle);
    }
}
