...
std::string tea_build_encrypt_path(const std::string& sourcePath) {
    ...
    std::string filename = tea_extract_filename(sourcePath);
...
{
    std::vector<uint8_t> data = tea_random_bytes(byteCount);
    if (!tea_write_file(filepath, data))
        return {};
                                         ...
                                         const std::string& label)
{
    ...
    if (!tea_read_file(filepath, data))
        return {};
